...
namespace AdventOfCode.Year2018
{
    class Day20
    {
        ...
        public void Part1()
        {
            Console.WriteLine(Traverse(0, Input));
        }

        public static int Traverse(int depth, string remaining)
        {
            if (remaining.Length == 0) return depth;
            ...
            switch (remaining[0])
            {
                ...
                case 'E':
                case 'W':
                    return Traverse(depth + 1, remaining.Substring(1));
                case '(':
                    int bracketIndentLevel = 1;
                    int index = 1;
                    int segmentStart = 1;
                    List<int> segementDepths = new List<int>();
                    ...
                    while (true)
                    {
                        ...
                        else if (remaining[index] == ')')
                        {
                            ...
                            if (bracketIndentLevel == 0)
                            {
                                ...
                                {
                                    int thisOptionDepth = Traverse(depth, remaining.Substring(segmentStart, index - segmentStart));
                                    segementDepths.Add(thisOptionDepth);
                                }
                                break;
                            }
                        }
                        else if (bracketIndentLevel == 1 && remaining[index] == '|')
                        {
                            int thisOptionDepth = Traverse(depth, remaining.Substring(segmentStart, index - segmentStart));
                            segementDepths.Add(thisOptionDepth);
                    ...
                    int maxCurrentDepth = emptySegment ? depth : segementDepths.Max();
                    return Traverse(maxCurrentDepth, remaining.Substring(index + 1));
                default:
    ...
    public class TestDay20
    {
        ...
        {
            Assert.AreEqual(3, Day20.Traverse(0, "WNE$"));
        }
        [TestMethod]
        public void SampleTest2()
        {
            Assert.AreEqual(10, Day20.Traverse(0, "ENWWW(NEEE|SSE(EE|N))$"));
        }
        [TestMethod]
        public void SampleTest3()
        {
            Assert.AreEqual(18, Day20.Traverse(0, "ENNWSWW(NEWS|)SSSEEN(WNSE|)EE(SWEN|)NNN$"));
        }
        [TestMethod]
        public void SampleTest4()
        {
            Assert.AreEqual(23, Day20.Traverse(0, "ESSWWN(E|NNENN(EESS(WNSE|)SSS|WWWSSSSE(SW|NNNE)))$"));
        }    
        [TestMethod]
        public void SampleTest5()
        {
            Assert.AreEqual(31, Day20.Traverse(0, "WSSEESWWWNW(S|NENNEEEENN(ESSSSW(NWSW|SSEN)|WSWWN(E|WWS(E|SS))))$"));
        }
    }
}
